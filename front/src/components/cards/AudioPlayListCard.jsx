...
const AudioPlayListCard = ({ id, thumbnail, title, type, createDate }) => {
    const formatedTitle = formatString(title);
    const formatedDate = formatDate(createDate);
    return (
        <Link to={`/playlist/video/${id}`} className="snap-center md:snap-start scroll-mx-6 shrink-0 w-[80%] sm:w-56 md:w-52 lg:w-60">
